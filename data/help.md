...
* ключевое слово `type` отвечает описанию сложного композиционного слоя (например, среда с экситонным возбуждением или системой наночастиц), заранее известного (см. ниже) или определенного пользователем
...
Каждая среда редактируется в своём поле (`Left Medium` / `Right Medium`) с той же
подсветкой синтаксиса, что и основное поле структуры. Удобный приём: скопировать
строку `material: ...` крайнего слоя структуры прямо в поле среды.
...
* `adachi_T` - расширенная модель Адачи для четверного сплава `Al(x)Ga(1-x)As(y)Sb(1-y)`, учитывающая зависимость оптических свойств компонент раствора от температуры [2]
    * `cAl` - концентрация `Al` в диапазоне от `0` до `1`
    * `cAs` - концентрация `As` в диапазоне от `0` до `1`
* `adachi_langer` - температурно-зависимая модель показателя преломления `AlGaAs`, уточняющая простую модель Адачи `adachi_simple` вблизи края запрещенной зоны [13]. Отличия от `adachi_simple`: (1) коэффициенты дисперсии `A0(x)`, `B0(x)` сделаны квадратичными по составу `x`; (2) ширина запрещенной зоны `E0(x, T)` температурно-зависима по формуле Варшни (с интерполяцией параметров между `GaAs` и `AlAs` по Вургафтману) - именно сдвиг края зоны дает основной вклад в температурную зависимость `n`; (3) добавлены малые поправки `C0·T + D0·T²`. Температура `T` берется из глобального параметра расчета (поле температуры в основных параметрах). В отличие от `adachi_simple`, модель не "зажимает" энергию на краю зоны, а аналитически продолжает функцию Адачи выше края (как на Fig. 5 статьи): ниже края зоны материал прозрачен (`k = 0`), а выше края мнимая часть `k` возникает естественно. Поэтому параметр `df` этой модели НЕ нужен. Область применимости: состав `x = 0–0.5`, температура `T = 4–295 K`, длина волны `λ` от края зоны до `1100 нм`. Пример: `material: AlGaAs, eps: adachi_langer, cAl: 0.3`
    * `cAl` - концентрация `Al` в диапазоне от `0` до `1`
...
13. Temperature-dependent refractive index of AlGaAs for quantum-photonic devices near the bandgap. AIP Advances 16, 055028 (2026). https://doi.org/10.1063/5.0316285. M. Langer, S. A. Dhurjati, M. Bauer, Y. G. Zena, A. Rahimi, R. Bassoli, F. H. P. Fitzek, O. G. Schmidt, C. Hopfmann
