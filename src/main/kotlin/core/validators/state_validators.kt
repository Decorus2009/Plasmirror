...
import core.structure.initStructure
...
fun initState() = when {
  initOpticalParameters() == SUCCESS && initStructure() == SUCCESS -> SUCCESS
  else -> FAILURE
}
...
private fun initRegime(): ValidationResult {
   ...
   */
  fun toDefaults() {
    ComputationParametersStorage.mode = "Reflectance"
    State.mode = modes[ComputationParametersStorage.mode]!!
  }

  try {
    State.mode = modes[ComputationParametersStorage.mode]!!
    // if something is wrong with regime in JSON file and no mapping is found
    // TODO FIX: KotlinNullPointerException
